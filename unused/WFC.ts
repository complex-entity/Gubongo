...
namespace WFC {
    export interface NeighborCompatibilityData {
        [key: string]: Set<string>;
    }

    export interface TileData {
        name: string;
        edges: string[]; // left bottom right top
    }
}

class WFC {
    private wave: Set<string>[];
    private propagating: Set<number>;
    private width: number;
    private height: number;
    private length: number;

    private neighborCompatibilityData: WFC.NeighborCompatibilityData;
    private tileDatas: { [key: string]: WFC.TileData };

    constructor(width: number, height: number, neighborData: WFC.NeighborCompatibilityData, tileDatas: WFC.TileData[]) {
        this.width = width | 0;
        ...
        this.propagating = new Set<number>();
        ...
        for (let type in neighborData) {
            this.neighborCompatibilityData[type] = new Set<string>();
        }
    ...
    // manually set a tile
    public SetConstraint(x: number, y: number, tileType: string | string[]) {
    ...
    private Collapse(x: number, y: number) {
        ...
        const collapse = (neighborX: number, neighborY: number, direction: number) => {
            ...
            const incompatibleTiles = new Set<string>();
            for (let tileType of neighborTiles) {
